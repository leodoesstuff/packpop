const STORE_KEY = "packpop_save_v2";
...
const PACKS = {
  common: { cost:100, pulls:5, odds:{ Common:.80, Rare:.17, Epic:.027, Legendary:.003 } },
  rare:   { cost:250, pulls:5, odds:{ Common:.60, Rare:.34, Epic:.05,  Legendary:.01  } },
  epic:   { cost:500, pulls:5, odds:{ Common:.40, Rare:.45, Epic:.13,  Legendary:.02  } },
};
...
const byRarity = (r)=> CARD_SET.filter(c=>c.rarity===r);
...
function load(){
  try{
    ...
    if(!parsed.daily) parsed.daily = { lastClaimDay:null, streak:0 };
    return parsed;
  }catch{
    return null;
  }
...
function claimDaily(){
  ...
  if(state.daily.lastClaimDay === yesterday) state.daily.streak = Math.min(state.daily.streak+1, 30);
  ...
  save();
  renderAll();
  toast(`+${reward} coins (streak ${state.daily.streak})`);
}
...
function openPack(packKey){
  ...
  for(let i=0;i<pack.pulls;i++){
    ...
    if(had >= 1){
      state.dust += (DUST_FROM_DUP[card.rarity] ?? 1);
    }
  ...
  // auto switch to Open tab so it feels good
  document.querySelector('.tabbtn[data-go="open"]')?.click();
}
...
function craft(cardId){
  ...
  if(state.dust < cost){ toast("Not enough dust."); return; }
...
function renderDailyInfo(){
  ...
  ui.dailyInfo.textContent = claimed ? `Claimed • streak ${state.daily.streak}` : `Ready • streak ${state.daily.streak || 0}`;
}
...
function renderCollection(){
  if(!ui.collection) return;

  const filter = ui.filterRarity?.value ?? "all";
  const sortBy = ui.sortBy?.value ?? "rarity";

  let items = CARD_SET.map(c=>({ ...c, count: state.owned[c.id] ?? 0 }));
  if(filter !== "all") items = items.filter(x=>x.rarity===filter);

  if(sortBy === "name") items.sort((a,b)=> a.name.localeCompare(b.name));
  else if(sortBy === "owned") items.sort((a,b)=> (b.count-a.count) || raritySort(a,b));
  else items.sort(raritySort);
  ...
  for(const c of items){
    ...
    tile.innerHTML=`
      ...
      <div class="meta"><span class="badge ${c.rarity}">${c.rarity}</span></div>
      <div class="cOwned">
  ...
  for(const c of items){
    ...
    el.innerHTML=`
      ...
      <div class="cOwned">
        ...
        <button class="btn ${can ? "" : "ghost"}" ${can ? "" : "disabled"} data-craft="${c.id}">Craft</button>
      </div>
    `;
